...
/**
 * Předindexace DZS SQLite databáze bez Pythonu.
 *
 * <pre>
 *   java -jar preindex-dzs.jar DZS_PASPORT_TPI.sqlite
...
public final class PreindexMain {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String idxName = DzsPreindexer.cacheFileName(dbFile);
            File idxFile = new File(outDir, idxName);
            DzsPreindexer.writeIndex(dbFile, result, idxFile);
            ...
            System.out.println("  RO záznamů:  " + result.ro.size());
            System.out.println("  GPS bodů:    " + result.gps.size());
            System.out.println("  Výhybky GPS: " + result.vyhybkaGps.size());
            System.out.printf("  Čas:         %.1f s%n", elapsed);
            System.out.println("  Otisk:       velikost=" + dbFile.length()
                    + ", mtime_ms=" + dbFile.lastModified());

            if (verify) {
                boolean ok = DzsPreindexer.verifyIndex(dbFile, idxFile);
    ...
    private static void printUsage() {
        System.out.println("Předindexace DZS databáze pro RFID Go GPS (bez Pythonu)");
        System.out.println();
        ...
        System.out.println();
        System.out.println("Na telefon zkopírujte DZS_PASPORT_TPI.sqlite a DZS_PASPORT_TPI.sqlite.idx");
        System.out.println("do stejné složky (např. Stažené soubory).");
    }
}
