...
- Po spuštění aplikace automaticky hledá **`DZS_PASPORT_TPI.sqlite`** ve složce **Stažené soubory**. Podporovány jsou i soubory s `DZS` / `PASPORT` v názvu.
...
Soubor: **`Download/rfid_go_gps_output.csv`** – přímo ve složce Stažené soubory (z PC přes USB zapisovatelné).

**Nahrání z PC přes USB:** V Průzkumníku Windows otevřete čtečku → vnitřní úložiště → **Download** (Stažené soubory) a soubor `rfid_go_gps_output.csv` tam zkopírujte nebo přepište. Složka Documents z MTP na čtečce Chainway C5 zapisovatelná není. Po nahrání vraťte se do aplikace nebo otevřete panel Pokročilé. Alternativa: tlačítko **Načíst CSV** v aplikaci.
