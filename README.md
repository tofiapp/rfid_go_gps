...
**Nahrání z PC přes USB:** V Průzkumníku Windows otevřete čtečku → vnitřní úložiště → **Download** (Stažené soubory) a soubor `rfid_go_gps_output.csv` tam zkopírujte nebo přepište. Po nahrání se vraťte do aplikace – CSV se automaticky načte. Alternativa: tlačítko **Načíst CSV** v panelu Pokročilé.
