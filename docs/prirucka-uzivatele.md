...
Pro tag na **hranici dvou úseků tratě** (mimo běžný cyklus výhybky):
...
3. Vyplňte **objekt** (kolej nebo výhybka), kde se tag nachází.
4. Kilometrickou polohu můžete doplnit později.
5. Po potvrzení zvolte režim čtení a načtěte tag spouštěm stejně jako u běžného čipu.
6. Po úspěšném zápisu aplikace sama skončí režim hranice a v GPS režimu najde **další výhybku**.

Režim hranice TUDU skončí také při ruční změně UDU/výhybky nebo po kliknutí **Načíst polohu**.

> Srozumitelnější návod pro terén: **`RFID_Go_GPS_hranice_TUDU.pdf`**
...
> Podrobný postup zápisu hranice TUDU je v **kapitole 5.8**. Samostatný srozumitelný návod pro terén je v PDF **`RFID_Go_GPS_hranice_TUDU.pdf`**.
...
Podrobný návod: **`RFID_Go_GPS_hranice_TUDU.pdf`** (zdroj: `docs/hranice-tudu.md`).
